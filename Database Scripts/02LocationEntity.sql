...
create or replace stream location_stm 
on table location
append_only = true
...
from (
    ...
    from @stage_csv/full-load/location t
)
...
create or replace stream restaurant_location_stm 
...
MERGE INTO restaurant_location AS target
USING (
    SELECT 
...
create or replace table restaurant_location_dim (
...
MERGE INTO 
        RESTAURANT_LOCATION_DIM AS target
    USING 
        RESTAURANT_LOCATION_STM AS source
    ON 
...
from (
    select 
        ...
        metadata$file_content_key as _stg_file_md5,
        current_timestamp as _copy_data_ts
    from @stage_csv/cdc/location/delta-day02-2rows-update.csv t
)
file_format = (format_name = 'csv_file_format')
on_error = abort_statement;
...
list @stage_csv/cdc/location/delta-day02-2rows-update.csv;
select * from location_stm;
